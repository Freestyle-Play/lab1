...
int main() {
    ...
    double tsum = M_PI * M_PI / 8; // Точное значение суммы ряда pi^2/8
    double eps;  // Переменная для хранения введённой точности
    double term; // Переменная для хранения текущего члена ряда
    double diff; // Разность между точной и насчитанной суммой 
    ...
    cout << "Разность с точным значением: " << setprecision(15) << tsum - sum << endl;
    // Вывод точного значения суммы ряда
    cout << "Точное значение: " << setprecision(15) << tsum << endl;
